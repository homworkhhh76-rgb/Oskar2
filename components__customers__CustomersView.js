...
import { exportToCSV, printReceiptElement } from './utils__export.js?v=7.9.4.41-recipe-accounting';
...
import { downloadProfessionalCustomerStatementPDF, downloadProfessionalCustomerStatementImage, downloadProfessionalCustomerStatementExcel, downloadProfessionalTablePDF, downloadProfessionalTableImage } from './utils__professionalExport.js?v=7.9.4.41-recipe-accounting';
import { createQrSvgDataUrl } from './utils__qrcode.js?v=7.9.4.41-recipe-accounting';
...
export const CustomersView = () => {
    ...
    // Get Invoices for customer statement
    const customerInvoices = statementCustomer
        ? invoices.filter((i) => i.customerId === statementCustomer.id)
        : [];
    const customerStatementPager = usePagination(customerInvoices, 50, statementCustomer?.id || 'none');
    ...
    const handleOpenCustomerPortal = async (cust) => {
        ...
        try {
            ...
            if (!runtime?.companyId || !readDb?.databaseURL || !readDb?.authToken) {
                showToast('بوابة العملاء تحتاج ملف دخول جديد يحتوي على توكن القراءة فقط من إدارة القواعد.', 'warning');
                return;
            }
            let current = cust;
            let createdNonce = false;
            if (!current.portalNonce) {
                createdNonce = true;
                current = { ...cust, portalNonce: makePortalNonce(), portalEnabled: true, portalUpdatedAt: new Date().toISOString() };
                await saveCustomer(current);
            }
            const code = await encodeCustomerPortalAccess({
                d: readDb.databaseURL,
                t: readDb.authToken,
                c: runtime.companyId,
                u: current.id,
                n: current.portalNonce,
            });
            const url = new URL('./customer.html', window.location.href);
            url.search = '';
            url.hash = code;
            const fullUrl = url.toString();
            const qrDataUrl = createQrSvgDataUrl(fullUrl, { size: 340, margin: 4, level: 'L' });
            setPortalModal({ customer: current, url: fullUrl, qrDataUrl });
            if (createdNonce && window.OscarCloudSync?.syncNow && navigator.onLine !== false) {
                Promise.resolve(window.OscarCloudSync.syncNow({ manual: false, force: true })).catch(() => {});
            }
        }
        catch (err) {
