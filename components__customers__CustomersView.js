...
import { downloadProfessionalCustomerStatementPDF, downloadProfessionalCustomerStatementImage, downloadProfessionalCustomerStatementExcel, downloadProfessionalTablePDF, downloadProfessionalTableImage } from './utils__professionalExport.js?v=7.9.4.36-customer-portal-stable-2';
import { Plus, Search, FileText, DollarSign, Download, Printer, Trash2, X, Edit, Image as ImageIcon, FileSpreadsheet, Link2, } from 'lucide-react';
...
export const CustomersView = () => {
    ...
    const handleCopyCustomerPortal = async (cust) => {
        try {
            ...
            if (!current.portalNonce) {
                ...
                if (window.OscarCloudSync?.syncNow && navigator.onLine !== false) {
                    try { await window.OscarCloudSync.syncNow({ manual: false, force: true }); } catch (_) { }
                }
            }
            ...
            showToast(`تم نسخ رابط العميل ${current.name}`, 'success');
        }
        catch (err) {
