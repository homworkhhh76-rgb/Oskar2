...
export const RestaurantWasteView = () => {
    ...
    const calculateRecipeDraftCost = (ingredients) => (ingredients || []).reduce((sum, ing) => {
    ...
    const draftRecipeCost = calculateRecipeDraftCost(recipeIngredients);
    const handleSaveRecipe = async (e) => {
        ...
        const savedCost = Number(savedRecipe?.totalCost ?? savedRecipe?.costPerBaseUnit ?? draftRecipeCost) || 0;
        showToast(`تم حفظ الوصفة وتحديث تكلفة ${prod.name} إلى ${savedCost.toFixed(2)} ₪`, 'success');
