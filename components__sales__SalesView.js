...
import { downloadProfessionalTablePDF, downloadProfessionalTableImage, downloadProfessionalTableExcel, downloadProfessionalInvoicePDF, downloadProfessionalInvoiceImage, downloadProfessionalInvoiceExcel } from './utils__professionalExport.js?v=7.9.4.41-recipe-accounting';
...
export const SalesView = () => {
    ...
    const paymentMethodLabel = (payment = {}) => {
        const rawName = String(payment.accountName || '').trim();
        const name = rawName.toLowerCase();
        const method = String(payment.method || '').toLowerCase();
        if (/jawwal|جوال/.test(name)) return 'Jawwal Pay';
        if (/palpay|pal pay|بال.?بي|بال.?باي/.test(name)) return 'PalPay';
        if (method === 'bank' || /بنك|bank/.test(name)) return rawName || 'بنك';
        if (method === 'wallet' || /محفظ/.test(name)) return rawName || 'محفظة إلكترونية';
        if (method === 'card' || /visa|master|بطاق|نقاط البيع/.test(name)) return rawName || 'بطاقة / POS';
        if (method === 'cash' || /صندوق|كاش|cash/.test(name)) return rawName || 'نقدي';
        return rawName || method || 'نقدي';
    };
    const formatInvoicePayment = (inv) => {
        const rows = Array.isArray(inv?.payments) ? inv.payments.filter((p) => Number(p?.amount) > 0 || p?.accountName || p?.method) : [];
        if (inv?.paymentType === 'debt' && !rows.length) return 'آجل';
        if (inv?.paymentType === 'customer_balance') return 'رصيد العميل';
        const labels = [...new Set(rows.map(paymentMethodLabel).filter(Boolean))];
        if (inv?.paymentType === 'multi') return labels.length ? `متعدد: ${labels.join(' + ')}` : 'دفع متعدد';
        if (inv?.paymentType === 'partial') return labels.length ? `جزئي: ${labels.join(' + ')}` : 'دفع جزئي';
        if (labels.length) return labels.join(' + ');
        return inv?.paymentType === 'cash' ? 'نقدي' : (inv?.paymentType || 'نقدي');
    };
    // Filter invoices
    ...
    const getSalesExportData = () => {
        const headers = ['رقم الفاتورة', 'النوع', 'التاريخ', 'العميل', 'نوع الدفع', 'الصنف', 'الوحدة', 'الكمية', 'السعر', 'إجمالي الصنف', 'إجمالي الفاتورة', 'المدفوع', 'المتبقي'];
        const rows = [];
        filteredInvoices.forEach((inv) => {
            ...
            items.forEach((item) => rows.push([
                ...
                new Date(inv.date).toLocaleString('ar-EG-u-nu-latn'),
                inv.customerName || 'زبون عام',
                formatInvoicePayment(inv),
                item?.productName || '-',
                item?.unitName || '-',
