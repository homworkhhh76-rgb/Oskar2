...
import { askOscar, getAIConfig, aiHealthCheck, fileToDataUrl } from './services__ai.js?v=7.9.4.11-notifications-popup';
import { executeAIActions, isMutationAction } from './services__aiActions.js?v=7.9.4.11-notifications-popup';
...
export const OscarAI = () => {
  ...
  const previews=useMemo(()=>files.map(f=>{const name=String(f?.name||'');const isPdf=/application\/pdf/i.test(f?.type||'')||/\.pdf$/i.test(name);const isExcel=/\.(xlsx|xls)$/i.test(name)||/spreadsheet|ms-excel/i.test(f?.type||'');return {file:f,isPdf,isExcel,url:(!isPdf&&!isExcel)?URL.createObjectURL(f):''};}),[files]);
  ...
  return h(React.Fragment,null,
    ...
    open&&h('div',{className:'oscar-ai-overlay fixed z-[2147481600] bg-black/45 backdrop-blur-[2px] flex justify-center',onClick:()=>setOpen(false)},
      h('section',{dir:'rtl',onClick:e=>e.stopPropagation(),className:'oscar-ai-panel w-full sm:max-w-3xl bg-white shadow-2xl overflow-hidden flex flex-col border border-slate-200'},
        ...
        showSettings&&h('div',{className:'shrink-0 p-3 border-b bg-slate-50 space-y-2'},h('div',{className:'text-xs font-black'},'اتصال Oscar AI'),h('div',{className:'rounded-xl border bg-white px-3 py-2 text-[11px] text-slate-600'},h('div',{className:'font-bold text-slate-700'},'المصدر: ملف دخول الشركة (.mzauth)'),h('div',{className:'mt-1 font-mono text-[10px]'},`الموديل: ${getAIConfig().model}`),h('div',{className:`mt-1 text-[10px] ${getAIConfig().apiKey?'text-emerald-600':'text-rose-600'}`},getAIConfig().apiKey?'مفتاح AI موجود في ملف الدخول ✓':'ملف الدخول لا يحتوي على مفتاح AI')),h('button',{type:'button',onClick:testConnection,className:'px-3 py-2 rounded-xl border bg-white text-xs font-bold'},'اختبار الاتصال'),connection?h('div',{className:'text-[10px] text-slate-500'},connection):null),
        h('div',{ref:scrollRef,className:'flex-1 min-h-0 overflow-y-auto overscroll-contain p-3 sm:p-4 space-y-3 bg-slate-50'},
          messages.length===0?h(React.Fragment,null,h('div',{className:'rounded-2xl bg-white border p-4 shadow-sm'},h('div',{className:'font-black text-sm flex items-center gap-2'},h(Sparkles,{className:'w-4 h-4 text-emerald-600'}),'أوسكار جاهز للتنفيذ'),...insights.map((x,i)=>h('div',{key:i,className:'mt-2 text-xs text-slate-600 flex gap-2'},h(CheckCircle2,{className:'w-4 h-4 text-emerald-500 shrink-0 mt-0.5'}),h('span',null,x))),h('div',{className:'mt-3 text-[11px] text-slate-500'},'اكتب طلبك أو أرفق ملفاً: صورة أو PDF أو Excel. يدعم إضافة الأصناف والمشتريات والعملاء والسندات والتقارير والمرتجعات.')),h('div',{className:'grid grid-cols-2 sm:grid-cols-4 gap-2'},...quicks.map(([q,label,Icon],i)=>h('button',{key:i,type:'button',onClick:()=>setInput(q),className:'p-3 rounded-2xl bg-white border text-right hover:border-emerald-300 shadow-sm'},h(Icon,{className:'w-5 h-5 text-emerald-600 mb-2'}),h('div',{className:'text-xs font-black'},label),h('div',{className:'text-[10px] text-slate-400 mt-1 line-clamp-2'},q))))):null,
        ...
        h('footer',{className:'shrink-0 p-3 border-t bg-white'},
          files.length?h('div',{className:'mb-2 flex gap-2 overflow-x-auto pb-1'},...previews.map((x,i)=>h('div',{key:i,className:'relative shrink-0 w-20 h-16 rounded-xl overflow-hidden border bg-slate-100'},x.isPdf?h('div',{className:'w-full h-full flex flex-col items-center justify-center gap-1 px-1 text-slate-600'},h(FileText,{className:'w-5 h-5 text-rose-500'}),h('span',{className:'text-[8px] text-center line-clamp-2'},x.file.name||'PDF')):x.isExcel?h('div',{className:'w-full h-full flex flex-col items-center justify-center gap-1 px-1 text-slate-600'},h(FileSpreadsheet,{className:'w-5 h-5 text-emerald-600'}),h('span',{className:'text-[8px] text-center line-clamp-2'},x.file.name||'Excel')):h('img',{src:x.url,className:'w-full h-full object-cover'}),h('button',{type:'button',onClick:()=>setFiles(prev=>prev.filter((_,idx)=>idx!==i)),className:'absolute top-1 left-1 w-5 h-5 rounded-full bg-black/65 text-white flex items-center justify-center'},h(X,{className:'w-3 h-3'}))))):null,
